...
with DAG(
    ...
    description='Process player stats data',
    ...
    def preprocess_batting_data(**kwargs):
        """Task to preprocess batting data."""
    ...
    def preprocess_bowling_data(**kwargs):
        """Task to preprocess bowling data."""
        spark = create_spark_session()
        base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
        raw_data_dir = os.path.join(base_dir, 'data', '1_rawData')
        processed_data_dir = os.path.join(base_dir, 'data', '2_processedData')
    ...
    def preprocess_fielding_data(**kwargs):
        """Task to preprocess fielding data."""
        spark = create_spark_session()
        base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
        raw_data_dir = os.path.join(base_dir, 'data', '1_rawData')
        processed_data_dir = os.path.join(base_dir, 'data', '2_processedData')
    ...
    # Define tasks
    batting_task = PythonOperator(
        task_id='preprocess_batting_data',
        python_callable=preprocess_batting_data,
        provide_context=True,
    )

    bowling_task = PythonOperator(
        task_id='preprocess_bowling_data',
        python_callable=preprocess_bowling_data,
        provide_context=True,
    )

    fielding_task = PythonOperator(
        task_id='preprocess_fielding_data',
        python_callable=preprocess_fielding_data,
        provide_context=True,
    )

    # Set task dependencies if necessary
    # Run tasks in parallel
    [batting_task, bowling_task, fielding_task]
