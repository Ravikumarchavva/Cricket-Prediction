...
def augment_sequences(match_sequences, labels, truncation_points=[6, 10, 15, 20, 25, 30, 35, 40]):
    """
    Augments match data by truncating sequences at various points to simulate predictions at different stages.
    
    Parameters:
    - match_sequences: List of match data sequences (e.g., ball-by-ball data).
    - labels: List of labels corresponding to match outcomes.
    - truncation_points: List of overs at which to truncate the sequence.

    Returns:
    - Augmented match_sequences and labels.
    """
    augmented_sequences = []
    augmented_labels = []
    ...
    for i, sequence in enumerate(match_sequences):
        for point in truncation_points:
            balls = point * 6  # Convert overs to balls
            if len(sequence) >= balls:
                augmented_sequences.append(sequence[:balls])
                augmented_labels.append(labels[i])  # Same label
